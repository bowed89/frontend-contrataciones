...
@Component({
  selector: 'app-form-tdr-editar-pre',
  templateUrl: './form-tdr-editar-pre.component.html',
  styleUrls: ['./form-tdr-editar-pre.component.css']
})
export class FormTdrEditarPreComponent implements OnInit {
      ...
      this._unidadSolicitanteService.getTdrId(this.id).subscribe(res => {
        ...
        for (let i in res) {
          ...
          this.tdrForm.patchValue({
            ...
            area: res[i].area,
            lugar_horario: res[i].lugar_horario,
            supervision_dep: res[i].supervision_dep,
            honorarios_me: res[i].honorarios_me
          })
        }
      })
    })
  ...
  onSubmit() {
    ...
    this._unidadSolicitanteService.putTdrPre(this.tdrForm.value, this.id).subscribe(res => {
      Swal.fire({
        icon: 'success',
        title: '¡Éxito!',
